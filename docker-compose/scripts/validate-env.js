...
// 1b) App-specific env (cronjob-manager: backend + frontend)
checkPort("BACKEND_PORT", true);
checkRequired("API_SECRET", "shared secret giữa frontend↔backend (x-api-secret)", (v) =>
  v.length >= 8 && v !== "change-me-super-secret" ? null : "phải >= 8 ký tự và không dùng giá trị mẫu",
);
// Firebase RTDB: bắt buộc trừ khi chạy chế độ memory.
if ((env.RTDB_MODE || "").toLowerCase() === "memory") {
  ok.push("RTDB_MODE=memory (in-process store, không cần Firebase)");
} else {
  checkRequired("FIREBASE_DB_URL", "Firebase RTDB URL", (v) =>
    v.startsWith("https://") ? null : "phải bắt đầu bằng https://",
  );
  const sa = (env.FIREBASE_SERVICE_ACCOUNT || "").trim();
  const secret = (env.FIREBASE_AUTH_SECRET || "").trim();
  if (!sa && !secret) {
    errors.push("Firebase auth thiếu: cần FIREBASE_SERVICE_ACCOUNT (ưu tiên) hoặc FIREBASE_AUTH_SECRET");
  } else {
    ok.push(`Firebase auth=${sa ? "service_account" : "auth_secret"}`);
  }
}
checkOptional("CRONJOB_API_BASE", "cronjob.org API base", (v) =>
  v.startsWith("http") ? null : "phải là URL http(s)",
);
checkOptional("RTDB_EXEC_QUEUE_PATH", "RTDB exec queue path", (v) => (v.startsWith("/") ? null : "phải bắt đầu bằng '/'"));
checkOptional("EXEC_TIMEOUT_MS", "per-exec timeout ms", (v) => {
  const n = Number(v);
