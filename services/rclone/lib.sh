...
#
#  Hỗ trợ CẤU HÌNH NHIỀU PATH qua indexed env vars:
#
...
rclone_collect_paths() {
  ...
  while [ "$_i" -le "$RCLONE_MAX_PATHS" ]; do
    ...
    # Một path hợp lệ cần CẢ local + remote. Quét hết tới MAX để tránh bỏ sót
    # do người dùng đánh số không liền (cho phép "lỗ hổng" index).
